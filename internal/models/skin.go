...
// GenerateMarketHashName создает marketHashName из weapon, name и quality
// Формат: "Weapon | Name (Quality)"
func GenerateMarketHashName(weapon, name, quality string) string {
	if quality == "" || quality == "None" {
		return fmt.Sprintf("%s | %s", weapon, name)
	}
	return fmt.Sprintf("%s | %s (%s)", weapon, name, quality)
}

func NewSkin(marketHashName, name, weapon, quality string) *Skin {
	now := time.Now()
	// Если marketHashName пустой, генерируем его автоматически
	if marketHashName == "" {
		marketHashName = GenerateMarketHashName(weapon, name, quality)
	}
	return &Skin{
		ID:             uuid.New(),
