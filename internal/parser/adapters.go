...
// SteamClientAdapter - адаптер для SteamClient
type SteamClientAdapter struct {
	client *SteamClient
}

func NewSteamClientAdapter(client *SteamClient) domain.MarketClient {
	return &SteamClientAdapter{client: client}
}

func (a *SteamClientAdapter) GetItemPrice(ctx context.Context, marketHashName string) (*domain.PriceData, error) {
	data, err := a.client.GetItemPrice(ctx, marketHashName)
	if err != nil {
		return nil, err
	}
...
func (a *SteamClientAdapter) SearchItems(ctx context.Context, query string) ([]domain.MarketItem, error) {
	items, err := a.client.SearchItems(ctx, query)
	if err != nil {
		return nil, err
	}

	result := make([]domain.MarketItem, len(items))
	for i, item := range items {
		result[i] = domain.MarketItem{
...
// KafkaProducerAdapter - адаптер для Kafka Producer
...
// RedisAdapter - адаптер для Redis
type RedisAdapter struct {
	redis *redis.Redis
}

func NewRedisAdapter(redis *redis.Redis) domain.CacheStorage {
...
// LoggerAdapter - адаптер для Logger
type LoggerAdapter struct {
	logger *logger.Logger
}

func NewLoggerAdapter(logger *logger.Logger) domain.Logger {
